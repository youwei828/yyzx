<?php
    ...
    $key = $_GET['key'];
    if (isset($key)){
        $resultlist11 = mysqli_query($conn,"SELECT * FROM videos WHERE videoname LIKE '%$key%'");
    }else{
        $resultlist11 = mysqli_query($conn,"SELECT * FROM videos ORDER BY vid DESC");
    }

    //分页
    $num = mysqli_num_rows($resultlist11);
    ...
    $resultlist123 = mysqli_query($conn,"SELECT * FROM videos WHERE videoname LIKE '%$key%' LIMIT $start,$rowpage");

    //右边栏排行
    $result1 = mysqli_query($conn,"SELECT * FROM videos ORDER BY downtimes DESC limit 4");
?>
            ...
            <div class="row">
                ...
                <div class="col-xs-12 col-lg-12 mlist">
                    ...
                    <ul class="list-inline row text-center">
                        ...
                        <?php
                            while($rowlist = mysqli_fetch_assoc($resultlist123)){
                        ?>
...
<!--                    分页部分-->
                    <nav aria-label="Page navigation" style="text-align: center">
                        <ul class="pagination">
                            <?php
                            if ($page > 1){
                                echo "<li><a href=search.php?key=$key&page=1>首页</a></li>";
                                echo "<li>
                    <a href=search.php?key=$key&page=".($page-1)." aria-label=\'Previous\'>
                    ...
                    <span aria-hidden=\'true\'>&laquo;</span>
                            ...
                            for ($i = 1;$i <= $totalpages;$i++){
                                ...
                                if ($suffix == $i)
                                    echo "<li class=\"active\"><a href=search.php?key=$key&page={$i}>{$i}</a></li>";
                                else
                                    echo "<li><a href=search.php?key=$key&page={$i}>{$i}</a></li>";
                            }
                            ?>
                            <?php
                            ...
                            if ($page < $totalpages){
                                echo "<li>
                    <a href=search.php?key=$key&page=".($page+1)." aria-label=\'Next\'>
                    <span aria-hidden=\'true\'>&raquo;</span>
                    </a>
                  </li>";
                                echo "<li><a href=search.php?key=$key&page=$totalpages>尾页</a></li>";
                            }else{
                                echo "<li class='disabled'>
                    <a aria-label=\'Next\'>
